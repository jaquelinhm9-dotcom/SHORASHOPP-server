...
function sanitizeCheckoutItem(item) {
    ...
    try {
      ...
      if (!items.length) {
        return res.status(400).json({
          ok: false,
          error:
            "No se recibieron productos válidos.",
        });
      }

      const successUrl =
        typeof req.body?.successUrl ===
          "string" &&
        /^https?:\/\//i.test(
          req.body.successUrl
        )
      ...
      if (!successUrl || !cancelUrl) {
        return res.status(400).json({
          ok: false,
          error:
            "Las URLs de regreso no son válidas.",
        });
      }
...
function verifyMercadoPagoSignature(req) {
  ...
  if (!xSignature || !xRequestId) {
    return {
      ...
      status: 401,
      reason:
        "Missing Mercado Pago signature headers",
    };
  }
  ...
  if (!ts || !v1) {
    return {
      ...
      status: 401,
      reason:
        "Invalid Mercado Pago signature format",
    };
  }
...
app.listen(
  ...
  () => {
    console.log(
      `SHORASHOPP server listening on port ${PORT}`
    );
  }
);
